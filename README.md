...
`ebbinghaus-review-planner` 是一个面向 ChatGPT / Codex 的学习与复习规划 Skill。它会把用户明确希望学习、记忆、复习、备考或长期掌握的资料，整理成知识地图、主动回忆题、自适应复习日期、每日学习任务，以及经过内容与逐页视觉审核的 A4 可打印学习手册。

当前版本：`1.2.0`

## 这个仓库是什么

这是 `艾宾浩斯复习规划大师` Skill 的 GitHub 一键安装仓库。
...
- Skill 作者：参考包含自适应复习规则、A4 成品工作流和质量审核脚本的完整 Skill。
...
- 标准版：完整知识地图、练习区、复习日历、自评区和独立答案区。
- 完整版：覆盖全部重要章节和 A/B 级知识点，长资料自动分册。

## 核心特点

- 不机械套用固定“遗忘曲线”，而是结合间隔重复、主动回忆、知识分级和错误反馈。
- 只有用户明确提出学习、记忆、复习、备考或长期掌握时才触发。
- 单纯总结、翻译、润色或文件提取不会误触发。
- A/B 级知识点必须有可检测的主动回忆题。
- 题目与答案分区，避免答案紧贴题目破坏回忆效果。
- 使用用户时区和具体日期生成复习计划。
- 默认每日预算 30 分钟，超出时优先保留 A 级知识点和薄弱点。
- 默认交付 A4 竖版 PDF 与 DOCX，不停留在普通聊天表格。
- PDF 会检查 A4 尺寸、近空白页、裁切、重叠、乱码和分页问题。
...
我完全不会这份资料，请帮我从零开始学习，生成知识地图、主动回忆题、具体复习日期，并制作成可以打印的 A4 学习手册。
```
...
4. 按知识类型生成主动回忆题和检测标准。
5. 计算具体复习日期和每日任务。
6. 根据 0–4 分掌握反馈动态调整复习间隔。
7. 审核知识点、题目、答案、编号、日期和原文位置。
8. 制作 A4 竖版 DOCX 母版并转换为 PDF。
9. 渲染并逐页检查裁切、重叠、乱码、断表、空白页和编号错位。
10. 修正后交付最终 PDF、DOCX 和用户要求的附加导出格式。
